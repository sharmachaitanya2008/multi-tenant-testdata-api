...
public class AllocatorService {
    ...
    public EnvironmentData allocate(String env, String category, String ownerId) {
        String c = coll(env);
        Query q = Query.query(Criteria.where("queryA.category").is(category)
                        .and("lease.claimedBy").is(null));
        Update u = new Update()
    ...
    public boolean release(String env, String id, String ownerId) {
        String c = coll(env);
        Query q = Query.query(Criteria.where("_id").is(id).and("lease.claimedBy").is(ownerId));
        Update u = new Update()
                .set("lease.claimedBy", null)
                .set("lease.claimedAt", null)
                .set("lease.expiresAt", null);
        var res = mongo.updateFirst(q, u, c);
        return res.getModifiedCount() > 0;
    }

    public boolean renewLease(String env, String id, String ownerId) {
        String c = coll(env);
        Query q = Query.query(Criteria.where("_id").is(id).and("lease.claimedBy").is(ownerId));
        Update u = new Update()
                .set("lease.expiresAt", Instant.now().plus(leaseMinutes, ChronoUnit.MINUTES));
    ...
    public EnvironmentData getById(String env, String id) {
        return mongo.findById(id, EnvironmentData.class, coll(env));
    }
